...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians, FrameHessian* fh_right,
                                      CalibHessian Hcalib) {
  // make coarse tracking templates for latstRef.
  memset(idepth[0], 0, sizeof(float) * w[0] * h[0]);
  memset(weightSums[0], 0, sizeof(float) * w[0] * h[0]);
  ...
  for (FrameHessian* fh : frameHessians) {
    for (PointHessian* ph : fh->pointHessians) {
      if (ph->lastResiduals[0].first != 0 &&
          ph->lastResiduals[0].second == ResState::IN)  // contains information about residuals to the last two (!)
                                                        // frames. ([0] = latest, [1] = the one before).
      {
        ...
        if (pt_track_right == ImmaturePointStatus::IPS_GOOD) {
          ImmaturePoint* pt_track_back =
              new ImmaturePoint(pt_track->lastTraceUV(0), pt_track->lastTraceUV(1), fh_right, &Hcalib);
...
void CoarseTracker::calcGSSSE(int lvl, Mat88& H_out, Vec8& b_out, SE3 refToNew, AffLight aff_g2l) {
  ...
  for (int i = 0; i < n; i += 4) {
    ...
    acc.updateSSE_eighted(
        ...
        _mm_sub_ps(zero,
                   _mm_add_ps(_mm_mul_ps(_mm_mul_ps(u, v), dx), _mm_mul_ps(dy, _mm_add_ps(one, _mm_mul_ps(v, v))))),
        ...
        _mm_mul_ps(a, _mm_sub_ps(b0, _mm_load_ps(buf_warped_refColor + i))), minusOne,
...
Vec6 CoarseTracker::calcRes(int lvl, SE3 refToNew, AffLight aff_g2l, float cutoffTH) {
  ...
  // printf("the Ki is:\n %f,%f,%f\n %f,%f,%f\n %f,%f,%f\n -----\n",Ki[lvl](0,0), Ki[lvl](0,1), Ki[lvl](0,2),
  // Ki[lvl](1,0), Ki[lvl](1,1), Ki[lvl](1,2), Ki[lvl](2,0), Ki[lvl](2,1), Ki[lvl](2,2) );
  Vec3f t = (refToNew.translation()).cast<float>();
  // printf("the t is:\n %f, %f, %f\n", t(0),t(1),t(2));
  Vec2f affLL =
      AffLight::fromToVecExposure(lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, aff_g2l).cast<float>();
...
void CoarseTracker::setCoarseTrackingRef(std::vector<FrameHessian*> frameHessians, FrameHessian* fh_right,
                                         CalibHessian Hcalib) {
  assert(frameHessians.size() > 0);
  lastRef = frameHessians.back();
    ...
    if (debugPrint) {
      Vec2f relAff =
          AffLight::fromToVecExposure(lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, aff_g2l_current)
              .cast<float>();
      printf("lvl%d, it %d (l=%f / %f) %s: %.3f->%.3f (%d -> %d) (|inc| = %f)! \t", lvl, -1, lambda, 1.0f, "INITIA",
             0.0f, resOld[0] / resOld[1], 0, (int)resOld[1], 0.0f);
      ...
      if (debugPrint) {
        Vec2f relAff =
            AffLight::fromToVecExposure(lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, aff_g2l_new)
                .cast<float>();
        printf("lvl %d, it %d (l=%f / %f) %s: %.3f->%.3f (%d -> %d) (|inc| = %f)! \t", lvl, iteration, lambda,
               extrapFac, (accept ? "ACCEPT" : "REJECT"), resOld[0] / resOld[1], resNew[0] / resNew[1], (int)resOld[1],
