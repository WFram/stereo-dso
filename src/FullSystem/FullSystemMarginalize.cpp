...
void FullSystem::flagFramesForMarginalization(FrameHessian* newFH) {
  ...
  for (int i = 0; i < (int)frameHessians.size(); i++) {
    ...
    if ((in < setting_minPointsRemaining * (in + out) ||
      ...
      //(int)fh->immaturePoints.size(), 					(int)fh->pointHessiansMarginalized.size(), (int)fh->pointHessiansOut.size(),
      //					visInLast, outInLast,
      ...
      //			printf("May Keep frame %d, as %'d/%'d points remaining (%'d %'d %'d %'d). VisInLast %'d
      /// %'d. traces %d, activated %d!\n", 					fh->frameID, in, in+out, 					(int)fh->pointHessians.size(),
      //(int)fh->immaturePoints.size(), 					(int)fh->pointHessiansMarginalized.size(), (int)fh->pointHessiansOut.size(),
      //					visInLast, outInLast,
      //					fh->statistics_tracesCreatedForThisFrame,
      //fh->statistics_pointsActivatedForThisFrame);
