...
PixelSelector::PixelSelector(int w, int h) : blockSize(32)
{
	...
	gradHist = new int[100*(1+w/blockSize)*(1+h/blockSize)];
	ths = new float[(w/blockSize)*(h/blockSize)+100];
	thsSmoothed = new float[(w/blockSize)*(h/blockSize)+100];
...
void PixelSelector::makeHists(const FrameHessian* const fh)
{
	...
	int wdiv = w/blockSize;
	int hdiv = h/blockSize;
	thsStep = wdiv;

	for(int y=0;y<hdiv;y++)
		for(int x=0;x<wdiv;x++)
		{
			float* map0 = mapmax0+blockSize*x+blockSize*y*w;
			int* hist0 = gradHist;// + 50*(x+y*wdiv);
			memset(hist0,0,sizeof(int)*50);

			for(int j=0;j<blockSize;j++) for(int i=0;i<blockSize;i++)
			{
				int it = i+blockSize*x;
				int jt = j+blockSize*y;
			...
			ths[x+y*wdiv] = computeHistQuantil(hist0,setting_minGradHistCut) + setting_minGradHistAdd;
		}
	...
	for(int y=0;y<hdiv;y++)
		for(int x=0;x<wdiv;x++)
		{
			...
			if(x>0)
			{
				if(y>0) 	{num++; 	sum+=ths[x-1+(y-1)*wdiv];}
				if(y<hdiv-1) {num++; 	sum+=ths[x-1+(y+1)*wdiv];}
				num++; sum+=ths[x-1+(y)*wdiv];
			}
			...
			if(y>0) 	{num++; 	sum+=ths[x+(y-1)*wdiv];}
			if(y<hdiv-1) {num++; 	sum+=ths[x+(y+1)*wdiv];}
			num++; sum+=ths[x+y*wdiv];

			thsSmoothed[x+y*wdiv] = (sum/num) * (sum/num);
