 ...
 *                       DO NOT USE [idepth], since it may be scaled with [SCALE_IDEPTH] ... however that is currently
 *set to 1 so never mind.
 ...
 *      ->maxRelBaseline: value roughly proportional to the relative baseline this point was observed with (0 = no
 *baseline). points for which this value is low are badly contrained.
...
class Output3DWrapper {
 public:
  ...
  /* Usage:
   * Called after each new Keyframe is inserted & optimized, with all keyframes that were part of the active window
   * during that optimization in [frames] (with final=false). Use to access the new frame pose and points. Also called
   * just before a frame is marginalized (with final=true) with only that frame in [frames]; at that point, no further
   * updates will ever occur to it's optimized values (pose & idepth values of it's points).
   *
  ...
  /* called once after a new keyframe is created, with the color-coded, forward-warped inverse depthmap for that
   * keyframe, which is used for initial alignment of future frames. Meant for visualization.
   *
   * Calling:
