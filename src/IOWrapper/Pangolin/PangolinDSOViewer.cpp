...
void PangolinDSOViewer::run() {
  ...
  pangolin::View& d_video_Right = pangolin::Display("imgKFDepth_Right").SetAspect(w / (float)h);
...
void PangolinDSOViewer::pushStereoLiveFrame(FrameHessian* image, FrameHessian* image_right) {
  ...
  for (int i = 0; i < w * h; i++) {
    internalVideoImg->data[i][0] = internalVideoImg->data[i][1] = internalVideoImg->data[i][2] =
        image->dI[i][0] * 0.8 > 255.0f ? 255.0 : image->dI[i][0] * 0.8;
    internalVideoImg_Right->data[i][0] = internalVideoImg_Right->data[i][1] = internalVideoImg_Right->data[i][2] =
